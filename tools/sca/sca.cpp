...
int main(int argc, char **argv) {
  ...
  // no MC or ASM output, still need to initialize
  InitializeAllTargets();
  InitializeAllTargetMCs();
  InitializeAllAsmPrinters();
  ...
  cl::ParseCommandLineOptions(argc, argv, "foobar\n");
  ...
  if (TheTarget == 0) {
    errs() << argv[0] << ": error selecting target for module '"
      << errstr << "'.\n";
    return 1;
  }

  std::auto_ptr<TargetMachine>
    tgtm(TheTarget->createTargetMachine(TheTriple.getTriple(),
                                        "", ""));
  assert(tgtm.get() && "Could not allocate target machine!");
  TargetMachine &TM = *tgtm.get();
